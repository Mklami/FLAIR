...
//		if (!f.exists()) f.mkdir();
		...
		// Check if sourceFileIndex uses "src.main.java." prefix (for path normalization)
		boolean usesSrcMainJava = false;
		if (sourceFileIndex != null && !sourceFileIndex.isEmpty()) {
			for (String indexedFile : sourceFileIndex.keySet()) {
				if (indexedFile.startsWith("src.main.java.")) {
					usesSrcMainJava = true;
					break;
				}
			}
		}
...
//				System.out.println(flag);
				...
				// Normalize path format to match sourceFileIndex
				// Git history may use "source/" or "src/java" while current repo uses "src/main/java/"
				if (usesSrcMainJava) {
					if (sourceFile.startsWith("source.")) {
						String packagePath = sourceFile.substring("source.".length());
						sourceFile = "src.main.java." + packagePath;
					} else if (sourceFile.startsWith("src.java.")) {
						String packagePath = sourceFile.substring("src.java.".length());
						sourceFile = "src.main.java." + packagePath;
					} else if (sourceFile.startsWith("java.")) {
						sourceFile = "src.main." + sourceFile;
					}
					if (sourceFile.startsWith("src.main.java.java.")) {
						sourceFile = sourceFile.replace("src.main.java.java.", "src.main.java.");
					}
				}
